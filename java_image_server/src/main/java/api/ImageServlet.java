...
public class ImageServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        // b) 通过 upload 对象进一步解析请求（解析 Http 请求中奇怪的 body 中的内容）
        ...
        try {
            ...
            // 告诉客户端出现的具体的错误是什么
            // 对于 JSON 格式的响应，需要把 content-type 设定成指定格式
        ...
        // 手动获取一下当前日期，并转成格式日期，yyMMdd => 20200514
        ...
        // 解决方法：让每次上传图片对应的路径都不相同（在路径中加上时间戳）
        ...
        // 看看数据库中是否存在相同的 MD5 值的图片，如果不存在，返回 null
        Image existImage=imageDao.selectByMd5(image.getMd5());

        imageDao.insert(image);

        // 2. 获取图片的内容属性，并且写入磁盘文件
        if(existImage==null){
    ...
    protected void doDelete(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        // 2. 创建 ImageDao 对象，查看到该图片对象对应的相关属性（为了知道这个图片对应的文件路径）
        ImageDao imageDao=new ImageDao();
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        /*
        resp.setStatus(200);     // 设置响应的状态
        // 把 hello 这个字符串放到 http 响应的 body 中了
    ...
    private void selectAll(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        // 只要把之前的相关的字段都约定成统一的命名，下面的操作就可以异步到位的完成整个转换
